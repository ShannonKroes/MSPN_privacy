...
def rdc_test(local_data, meta_types, domains, k=None, s=1.0 / 6.0, non_linearity=np.sin, n_jobs=-1, rand_gen=None):
    ...
    for (i, j), rdc in zip(pairwise_comparisons, rdc_vals):
        ...
        rdc_adjacency_matrix[i, j], p_values[i,j] = rdc
        rdc_adjacency_matrix[j, i], p_values[i,j] = rdc
    #
...
def getIndependentRDCGroups_py(
    ...
    print("raw pearson")
    print(pearsonr(local_data.T[0], local_data.T[1]))
    ...
    print(p_values > threshold)
    print(p_values )
    print(threshold)
    print(test)
    print(rdc_adjacency_matrix.shape)
    ...
    print("adjacency matrix AFTER THRESHOLD")
    print(rdc_adjacency_matrix)
...
def get_split_rows_univ(n_clusters=2, ohe=True, k=10, s=1 / 6, non_linearity=np.sin, n_jobs=-2, rand_gen=None, pre_proc=None):
