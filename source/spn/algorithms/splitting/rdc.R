...
rdc_p_value <- function(x,y,s=1/6,f=sin) {
  if(var(x) == 0|| var(y) == 0) {
    return(0)
  }
  x <- cbind(apply(as.matrix(x),2,function(u)ecdf(u)(u)),1)
  ...
  n<- nrow(x)
    ...
    cancor_xy <- cancor(cbind(f(x),1),cbind(f(y),1))
    cancor_yx <- cancor(cbind(f(y),1),cbind(f(x),1))
    
    pvalue_xy<- cor.test(rep(cancor_xy$xcoef, n)*x, rep(cancor_xy$ycoef, n)*y)
    pvalue_yx<- cor.test(rep(cancor_yx$xcoef, n)*y, rep(cancor_yx$ycoef, n)*x)
    
  return(min(pvalue_xy,pvalue_yx))
}



testRDC <- function(data, ohe, featureTypes, linear, test) {
  ...
  rdccoef <- foreach(i = 1:nrow(inputpos), .combine=rbind) %dorng% {
    ...
    if(test=="rdc"){rdcv1 <- rdc(d1,d2, linear=linear) }
    if(test=="p_value"){rdcv1 <- rdc_p_value(d1,d2) }
