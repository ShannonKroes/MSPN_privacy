...
def create_histogram_leaf(data, ds_context, scope, alpha=1.0, hist_source="numpy", bins_np="auto"):
    ...
    if data.shape[0] == 0:
        ...
        breaks, densities, repr_points = getHistogramVals(data, meta_type, domain, source=hist_source, bins_np=bins_np)
...
def getHistogramVals(data, meta_type, domain, source="numpy", bins_np="auto"):
    ...
    if source == "numpy":
        densities, breaks = np.histogram(data, bins=bins_np, density=True)
        mids = ((breaks + np.roll(breaks, -1)) / 2.0)[:-1]
        return breaks, densities, mids
